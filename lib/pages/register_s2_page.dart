...
import '../responsity/ChuKhachSanResponsity.dart';
import '../values/app_assets.dart';
import '../values/app_color.dart';
import '../values/app_styles.dart';

class Register_s2_page extends StatefulWidget {
  const Register_s2_page(
      {Key? key,
      required this.Email,
      required this.Password,
      required this.AdminKS})
      : super(key: key);
  final String Email;
...
class _Register_s2_pageState extends State<Register_s2_page> {
  ...
  void _checkInformation(String Email, String Password, String Hoten,
      String cmnd, String phonenumber, String ngaysinh, bool AdminKS) async {
      ...
      if (cmnd.length > 10 && Validate().isValidPhoneNumber(phonenumber)) {
        if (AdminKS == false) {
          KhachHang khachHang = new KhachHang(
              ...
              cmnd: cmnd,
              SDT: phonenumber,
              ChuoiNgaySinh: ngaysinh);
          try {
            // print(khachHang);
            bool a = await insertKhachHang(khachHang);
            if (a == true) {
              final tam =
                  await SharedPreferencesService.saveKhachHang(khachHang);
          ...
          try {
            // print(khachHang);
            bool a = await insertChuKhachHang(chuKhachSan);
            chuKhachSan = await getChuKhachSan(Email);
            if (a == true) {
              Navigator.pushAndRemoveUntil(
                  context,
                  MaterialPageRoute(
        ...
        if (cmnd.length > 10) errorCmnd = "CMND is lenght less 10 ";
          ...
          Expanded(
            ...
            onPressed: () {
              ...
              _checkInformation(
                  Email,
                  Password,
                  hotenController.value.text,
                  cmndController.value.text,
                  phoneController.value.text,
                  ngaySinhController.value.text,
                  AdminKS);
            },
